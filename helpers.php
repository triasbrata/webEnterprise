...
if (! function_exists('asset')) {
    ...
    {
        return url(str_finish('public','/').$path);
...
if (! function_exists('url')) {
    ...
    function url($path = null, $parameters = [], $secure = null)
    {
        if($_SERVER['SERVER_PORT'] != '80'){
            return 'http://'.str_finish($_SERVER['HTTP_HOST'],'/').$path; 
        }
        $host = 'http://'.str_finish($_SERVER['HTTP_HOST'],'/');
        $host_doc = str_replace(['index.php','/'], '', $_SERVER['SCRIPT_NAME']);
        return str_finish($host.$host_doc,'/').$path; 
